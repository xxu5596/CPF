...
from hocontact.postprocess.geo_optimizer_modified import GeOptimizer, init_runtime_viz, update_runtime_viz
...
import os
'''os.chdir('/home/xingyu/project/ContactOpt')
...
#from contactopt.run_contactopt_xingyu import run_contactopt
from contactopt.get_data_xingyu import create_dataset,get_contact_target

os.chdir('/home/xingyu/project/CPF')
'''
np.seterr(all="raise")
...
def process_result(results, vc_thresh, mano_root, anchor_root):
    ...
    for p_idx in range(n_points):
        ...
        sample_res.append(p_res)
                                           #####################################################################
                                            #################choose if to padd vertices and anchor(here is yes)
...
def picr_stage(args):
    ...
    # print all the infos about the example dataset
    ioutils.print_query(example_dataset.queries, desp="ho3d_dataset_queries")
    ...
    if args.init_ckpt is None:
        ...
        exit()
    map_location = f"cuda:{rank}"  # GPU 0
    ...
    # ==================== dumping train >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # epoch pass
    epoch_res = epoch_pass(rank, "ho3d", example_loader, model, epoch=0)
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
def run_sample(args,
    device,
    hoptim,
    info,  #intermediate[index]
    hodataset,   #from dataset  gt
    index,  #0 ~ dataset-1
    kmetric,
    ...
    # ==================== preparation stage >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # region 
    obj_verts_3d_adapt_np = hodataset.get_obj_verts_transf(index)     #obj_verts_gt ?no    -bbox_center(x)
    obj_verts_3d_np = np.asarray(info["obj_verts_3d"])                #obj_verts_curr
    #obj_verts_gt = torch.Tensor(obj_verts_3d_adapt_np).unsqueeze(0).to(device)
    obj_verts_curr = torch.Tensor(obj_verts_3d_np).unsqueeze(0)

    # obj: dumped trimesh & normals & tsl & rot
    obj_faces_np = np.asarray(hodataset.get_obj_faces(index))   #ho3d.py  line 351
    ...
    '''
    #add by xingyu: 
    # extrat obj_mesh for contactopt
    seq,im_idx = hodataset.idxs[index]
    annot =hodataset.seq_map[seq][im_idx]
    obj_id=annot['objName']

    if obj_id == '037_scissors':
        is_thin = torch.Tensor([1]).to(device) #penetration is not allowed.
    else:
        is_thin = torch.Tensor([0]).to(device)

    obj_model_dir = 'data/YCB_models_supp'
    obj_model_path = os.path.join(obj_model_dir, obj_id)
    obj_model_path = os.path.join(obj_model_path,'textured_simple_ds.obj')
    mesh = trimesh.load(obj_model_path)
    obj_faces = mesh.faces

    handBeta = annot['handBeta']
    handTrans = annot['handTrans']
    ############################################################################

    rot_matrix = SO3.exp(info["obj_rot"]).as_matrix()
    obj_normals_np = (rot_matrix @ (hodataset.get_obj_normal(index)).T).T   #obj_normal_curr
    obj_normals_curr = obj_normals_np
    ...
    obj_verts_3d_can_np = np.asarray(hodataset.get_obj_verts_can(index)[0]) #  obj_verts_gt ? yes (-bbox_center)
    obj_verts_gt = torch.Tensor(obj_verts_3d_np).unsqueeze(0).to(device)
    obj_normals_can_np = np.asarray(hodataset.get_obj_normal(index))        # obj_normal_gt      # ho3d.py line 359
    obj_normals_gt = torch.Tensor(obj_normals_can_np).unsqueeze(0).to(device)
    ...
    hand_verts_adapt_np = np.asarray(hodataset.get_hand_verts3d(index))      #hand_verts_gt
    hand_verts_gt =  torch.Tensor(hand_verts_adapt_np).unsqueeze(0).to(device)
    hand_joints_adapt_np = np.asarray(hodataset.get_joints3d(index))


    # hand: faces (adapt) & pose (dumped)
    hand_faces_np = np.asarray(hodataset.get_hand_faces(index))  #for hand reconstruction
    # layer = ManoLayer(
    #         joint_rot_mode="axisang", use_pca=False, mano_root="assets/mano", center_idx=None, flat_hand_mean=True,
    #     )
    # hand_faces = np.array(layer.th_faces).astype(np.long)       
    hand_pose_axisang_adapt_np = np.asarray(hodataset.get_hand_pose_wrt_cam(index))  #(4,4)
    hand_pose_np = np.asarray(info["hand_pose"])   # array #(16,4)
    hand_pose = torch.from_numpy(hand_pose_np).float().to(device)   #tensor
    hand_pose_axisang_np = quaternion_to_angle_axis(hand_pose).detach().cpu().numpy()   # array #hand_pose(16,3)

    # hand: verts & joints dumped
    hand_verts_np = np.asarray(info["hand_verts_3d"])   #hand_verts_curr
    hand_verts_curr =torch.Tensor(hand_verts_np).unsqueeze(0).to(device)
    ...
    hand_close_faces_np = np.array(hand_closed_trimesh.faces)  # for evaluation
    
    ########################################################################
    # #ADD BY XINGYU: (for constructing ho_gt and ho_aug)
    # hand_shape_aug = info['hand_shape']##   array
    # #print(handregister_key_callback_shape_aug)                             # from info
    # hand_pose_flatten = hand_pose_axisang_np.flatten() #(48,) array
    # hand_pose_pca_aug = util.fit_pca_to_axang(hand_pose_flatten, hand_shape_aug)  #18
    # #print(hand_pose_flatten.shape, hand_shape_aug.shape)
    # #print(hand_pose_pca_aug)
    # hand_pose_aug = hand_pose_flatten
    # hand_tsl_aug = info['hand_tsl']## (3,)
    # hand_mTc_aug = np.eye(4)
    # hand_mTc_aug[:3, 3] = hand_tsl_aug
    # obj_faces_aug = obj_faces_np
    # obj_verts_curr = obj_verts_3d_np
    # hand_shape_np = np.asarray(info["hand_shape"])
    # SAMPLE_VERTS_NUM = 2048
    # sample_idx = np.random.randint(0, len(obj_verts_curr), SAMPLE_VERTS_NUM)
    # sampled_obj_verts_curr= obj_verts_curr[sample_idx, :]

    # obj_mesh_curr = Meshes(verts=obj_verts_curr,faces=obj_faces)
    # obj_normals_curr = obj_mesh_curr.verts_normals_padded()
    # obj_normals_curr = torch.nn.functional.normalize(obj_normals_curr, dim=2, eps=1e-12)    # Because buggy mistuned value in Pytorch3d, must re-normalize
    # norms = torch.sum(obj_normals_curr * obj_normals_curr, dim=2)  # Dot product
    # obj_normals_curr[norms < 0.8] = 0.6   # TODO hacky get-around when normal finding fails completely
    # obj_normals_curr = obj_normals_curr.detach().squeeze().numpy()
    # sampled_obj_normals_curr = obj_normals_curr[sample_idx,:]

    # # from dataset
    # hand_shape_gt = handBeta
    # hand_pose_gt = hand_pose_axisang_adapt_np #(48.)
    # hand_pose_pca_gt = util.fit_pca_to_axang(hand_pose_gt, hand_shape_gt)  #(18,)=(3+15)
    # #print(hand_pose_pca_gt)
    # hand_tsl_gt = handTrans #(3,)
    # obj_faces_gt = obj_faces_np
    # obj_verts_gt = obj_verts_3d_adapt_np
    # hand_verts_gt = hand_verts_adapt_np
    # #print('obj_verts:',obj_verts_gt)

    # obj_mesh_gt = Meshes(verts=obj_verts_gt,faces=obj_faces)
    # obj_normals_gt = obj_mesh_gt.verts_normals_padded()
    # obj_normals_gt = torch.nn.functional.normalize(obj_normals_gt, dim=2, eps=1e-12)    # Because buggy mistuned value in Pytorch3d, must re-normalize
    # norms = torch.sum(obj_normals_gt * obj_normals_gt, dim=2)  # Dot product
    # obj_normals_gt[norms < 0.8] = 0.6   # TODO hacky get-around when normal finding fails completely
    # obj_normals_gt = obj_normals_gt.detach().squeeze().numpy()
    # sampled_obj_normals_gt = obj_normals_gt[sample_idx,:]

    #                                                        # False
    # mano_model = ManoLayer(mano_root='mano/models', use_pca=True, ncomps=ncomps, side='right', flat_hand_mean=False).to(device)
    # hand_verts_curr, hand_joints_curr = util.forward_mano(mano_model, hand_pose_pca_aug, hand_shape_np, [hand_mTc_aug])
    # batch_size = hand_pose_np.shape[0]
    # mano_mesh_gt = Meshes(verts=hand_verts_gt, faces=mano_model.th_faces.repeat(batch_size, 1, 1))
    # hand_normals_curr = mano_mesh_gt.verts_normals_padded()

    # hand_pose_pca_gt = util.fit_pca_to_axang(hand_verts_adapt_np, hand_shape_aug)
    # hand_verts_gt, hand_joints_gt = util.forward_mano(mano_model, hand_pose_pca_aug, hand_shape_np, [hand_mTc_aug])
    # batch_size = hand_pose_np.shape[0]
    # mano_mesh_gt = Meshes(verts=hand_verts_gt, faces=mano_model.th_faces.repeat(batch_size, 1, 1))
    # hand_normals_curr = mano_mesh_gt.verts_normals_padded()
    SAMPLE_VERTS_NUM =2048
    ...
    sampled_obj_verts_curr= obj_verts_curr[sample_idx, :]
    sampled_obj_normals_curr = obj_normals_curr[sample_idx,:]
    sampled_obj_verts_gt= obj_verts_gt[:,sample_idx, :]
    sampled_obj_normals_gt = obj_normals_gt[:,sample_idx,:]

    mano_model = ManoLayer(mano_root='mano/models', use_pca=True, ncomps=15, side='right', flat_hand_mean=False).to(device)
    
    batch_size = 1
    
    mano_mesh_gt = Meshes(verts=hand_verts_gt, faces=mano_model.th_faces.repeat(batch_size, 1, 1))
    hand_normals_gt = mano_mesh_gt.verts_normals_padded()

  
    mano_mesh_curr = Meshes(verts=hand_verts_curr, faces=mano_model.th_faces.repeat(batch_size, 1, 1))
    hand_normals_curr = mano_mesh_curr.verts_normals_padded()
    
    obj_contact_target, hand_contact_target = calculate_contact.calculate_contact_capsule(hand_verts_gt, hand_normals_gt, sampled_obj_verts_gt, sampled_obj_normals_gt,
                              caps_top=args.caps_top, caps_bot=args.caps_bot, caps_rad=args.caps_rad, caps_on_hand=args.caps_hand, contact_norm_method=args.cont_method)         
    
    # obj_contact_curr, hand_contact_curr = calculate_contact.calculate_contact_capsule(hand_verts_curr, hand_normals_curr, sampled_obj_verts_curr, sampled_obj_normals_curr,
    #                           caps_top=args.caps_top, caps_bot=args.caps_bot, caps_rad=args.caps_rad, caps_on_hand=args.caps_hand, contact_norm_method=args.cont_method)
    
    # loss_criterion = torch.nn.L1Loss(reduction='none')

    # contact_obj_sub = obj_contact_target - obj_contact_curr
    # contact_obj_weighted = contact_obj_sub + torch.nn.functional.relu(contact_obj_sub) * args.w_cont_asym  # Loss for 'missing' contact higher
    # loss_contact_obj = loss_criterion(contact_obj_weighted, torch.zeros_like(contact_obj_weighted)).mean(dim=(1, 2))

    # contact_hand_sub = hand_contact_target - hand_contact_curr
    # contact_hand_weighted = contact_hand_sub + torch.nn.functional.relu(contact_hand_sub) * args.w_cont_asym  # Loss for 'missing' contact higher
    # loss_contact_hand = loss_criterion(contact_hand_weighted, torch.zeros_like(contact_hand_weighted)).mean(dim=(1, 2))

    # loss = loss_contact_obj * args.w_cont_obj + loss_contact_hand * args.w_cont_hand
    # it = index #added
    # if args.w_pen_cost > 0 and it >= args.pen_it:
    #     pen_cost = calculate_contact.calculate_penetration_cost(hand_verts_curr, hand_normals_curr, sampled_obj_verts_curr, sampled_obj_normals_curr, is_thin, args.cont_method)
    #     loss += pen_cost.mean(dim=1) * args.w_pen_cost

    '''    
    # it is a list
    new_sample = create_dataset(hand_shape_aug,hand_pose_pca_aug,hand_tsl_aug,obj_faces_aug,obj_verts_aug,hand_faces_np,
                                hand_shape_gt,hand_pose_pca_gt,hand_tsl_gt,obj_faces_gt,obj_verts_gt,hand_verts_gt)
    #store ho_aug,ho_gt,obj_sampled_idx,hand_feats_aug,obj_feats_aug in it

    data_new,obj_contact_target,hand_contact_target = get_contact_target(args,new_sample)
    # store everything in the loader 
    # print(obj_contact_target,hand_contact_target)

    # data_new is a list with length of dataset here we assume 1. 
    # it has all the attributes in contactopt/loader.py
    ##########################################################################
    '''
    ...
    # hand_faces = hand_close_faces_np
    # hand_verts_gt = hand_verts_adapt_np
    ...
    # # 进入交互模式
    # vis_cur.run()
    # vis_cur.destroy_window()


    # no viz required
    if not viz_initialized:
        runtime_viz = init_runtime_viz(
            hand_verts_adapt_np,  #hand_verts_gt   from dataset
            hand_verts_np,        #hand_verts_init from info
            obj_verts_3d_adapt_np,#obj_verts_gt    from dataset
            hand_faces_np,        #hand_faces      from dataset
            obj_verts_3d_np,      #obj_verts_cur   from info
            obj_faces_np,         #obj_face_cur    from dataset
            contact_info=info,
        )
    ...
    opt_val_kwargs = dict(
        ...
        hand_pose_gt=([0], hand_pose[0:1, :]),   #(0,4) from info 
        hand_pose_init=(list(range(1, 16)), hand_pose[1:, :]), # (1:15,4) from info
        
        # obj
        obj_verts_3d_can=torch.from_numpy(obj_verts_3d_can_np).float().to(device),
        obj_normals_can=torch.from_numpy(obj_normals_can_np).float().to(device),  # for repulsion loss
        ...
        # data=data_new[0],  #from contactopt
        # it=index
        is_thin= is_thin

    )

    hoptim.set_opt_val(**opt_val_kwargs)# set the params to optimize

    hoptim.optimize(progress=True) #### set the loss function and do the loss.backward etc. ...
...
if __name__ == "__main__":
    ...
    parser.add_argument('--w_cont_obj', type=float, help='Weight of the obj contact in optimization') #added
    ...
    ## add:
    #parser.add_argument("--load_fhb_path", type=str, default="assets/mano/MANO_RIGHT.pkl")
    ...
    #args.test_dataset = new_sample
    args.split = 'im'

    intermediate=main(args)
