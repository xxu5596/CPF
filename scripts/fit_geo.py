...
def run_sample_by_idx(
    ...
    #common/optimize/HO3D/official/lcl_lrl_rq_rt_ctsl/hand_obj/{idx}_save.pkl
    ...
    hand_joints_0 = torch.from_numpy(hand_joints_np[0, ...])# 'index 0' from the first dimension
...
def worker(
    ...
    hoptim = GeOptimizer(
        device,
        lr=lr,
        n_iter=n_iter, #default 400 for ho3d :500
        verbose=False,
        mano_root=mano_root,
...
def main(
    ...
    for worker_id in range(n_workers):
        ...
        #device_count = 4             cuda_idx = worker_id mod device_count
        #worker_id=0,4,8,12,16,20,24->cuda0
        #          1,5,9,....->cuda1
        #          2,6,...   ->cuda2

    # initial jobs
    # parallel(n_jobs=?)(delayed(func))  to help reduce computation time
    collected = Parallel(n_jobs=n_workers)(
        delayed(worker)(
            device=device_list[worker_id],
            worker_id=worker_id,  # 0~23
...
if __name__ == "__main__":
    ...
    if not args.hodata_use_cache and not args.hodata_no_use_cache:
        g_hodata_use_cache = True   #neither -- true
    elif not args.hodata_use_cache and args.hodata_no_use_cache:
        g_hodata_use_cache = False  #no  -- false
    elif args.hodata_use_cache and not args.hodata_no_use_cache:
        g_hodata_use_cache = True   #yes -- true
    else:
        g_hodata_use_cache = True   #both -- true
    ...
    main(
        ...
        hodata_path=args.hodata_path,# data
        anchor_path=args.anchor_path,
        palm_path=args.palm_path,
        hodata_use_cache=g_hodata_use_cache,  #####cache
